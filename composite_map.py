...
def find_waypoints(routes_lonlat, proj, tolerance_m, min_unique_m, cluster_dist_m):
    ...
    for i in range(len(candidates)):
        ...
        for j in range(i + 1, len(candidates)):
            if used[j]:
                continue
            if np.linalg.norm(cand_m[i] - cand_m[j]) <= cluster_dist_m:
                group.append(j)
                used[j] = True
...
def _build_info_table_rows(waypoint_annotations, rest_stops, colors):
    ...
    for rs in sorted(rest_stops, key=_rs_num):
        ...
        routes_str = ", ".join(sorted(set(r.upper() for r in rs['routes'])))
...
def main():
    ...
    parser.add_argument('--waypoint-cluster-m', type=float, default=100.0,
                         help='Waypoint candidates within this distance (meters) '
                              'of each other are merged into a single marker '
                              '(default: 100)')
